...
function setSelectedStock(stock) {
  ...
  stockName.textContent = COMPANY_NAMES[stock.symbol] || stock.symbol;
  stockSymbol.textContent = stock.symbol;
  stockExchange.textContent = `${stock.symbol} • US MARKET`;
  stockPrice.textContent = formatMoney(stock.price);
  stockChange.textContent = `${change >= 0 ? '+' : ''}${change.toFixed(2)}% today`;
  stockChange.className = change >= 0 ? 'positive' : 'negative';
  stockStatus.textContent = 'Live quote';
  document.querySelectorAll('.price-card-item').forEach(card => card.classList.toggle('selected', card.dataset.symbol === stock.symbol));
  loadHistory(stock.symbol, selectedPeriod);
}
...
function renderPriceCard(stock) {
  ...
  card.addEventListener('click', () => setSelectedStock(stock));
  return card;
}
...
function filterPriceCards() {
  ...
  document.querySelectorAll('.price-card-item').forEach(card => {
    ...
    card.hidden = Boolean(query) && !symbol.includes(query) && !name.toUpperCase().includes(query);
  });
}

function renderMovers() {
  if (!movers) return;
  const gainers = [...stocks].sort((a, b) => Number(b.change || 0) - Number(a.change || 0));
  const losers = [...stocks].sort((a, b) => Number(a.change || 0) - Number(b.change || 0));
  const mode = document.querySelector('.toggle button.selected')?.dataset.mover || 'gainers';
  const list = (mode === 'losers' ? losers : gainers).slice(0, 5);
  movers.innerHTML = list.map(stock => {
    const change = Number(stock.change || 0);
    return `<button class="mover-row" type="button" data-symbol="${stock.symbol}"><span><b>${stock.symbol}</b><small>${COMPANY_NAMES[stock.symbol] || 'US Equity'}</small></span><span><strong>${formatMoney(stock.price)}</strong><em class="${change >= 0 ? 'positive' : 'negative'}">${change >= 0 ? '+' : ''}${change.toFixed(2)}%</em></span></button>`;
  }).join('');
  movers.querySelectorAll('.mover-row').forEach(row => row.addEventListener('click', () => {
    const stock = stocks.find(item => item.symbol === row.dataset.symbol);
    setSelectedStock(stock);
    document.getElementById('chart-card')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }));
}
...
async function loadPrices() {
  if (!priceGrid) return;
  try {
    pricesStatus.textContent = 'Updating...';
    const response = await fetch(apiUrl('/api/latest-prices'), { cache: 'no-store' });
    if (!response.ok) throw new Error(`Prices API returned ${response.status}`);
    const data = await response.json();
    stocks = Array.isArray(data.data) ? data.data : [];
    priceGrid.innerHTML = '';
    stocks.forEach(stock => priceGrid.appendChild(renderPriceCard(stock)));
    pricesStatus.textContent = `${stocks.length} stocks • updated ${formatTime()}`;
    marketUpdated.textContent = `Last update ${formatTime()}`;
    renderMovers();
    filterPriceCards();
    if (!selectedStock || !stocks.some(stock => stock.symbol === selectedStock.symbol)) {
      setSelectedStock(stocks[0]);
    } else {
      const fresh = stocks.find(stock => stock.symbol === selectedStock.symbol);
      setSelectedStock(fresh);
    }
  } catch (error) {
    console.error('Failed to load live prices:', error);
    pricesStatus.textContent = 'Unable to load live prices';
  }
}
...
function createChart() {
  ...
  marketChart = LightweightCharts.createChart(chartContainer, {
    autoSize: true,
    layout: { background: { type: 'solid', color: 'transparent' }, textColor: '#71817a' },
    grid: { vertLines: { color: '#edf2ef' }, horzLines: { color: '#edf2ef' } },
    rightPriceScale: { borderColor: '#dfe8e3' },
    timeScale: { borderColor: '#dfe8e3', timeVisible: true, secondsVisible: false },
    crosshair: { mode: LightweightCharts.CrosshairMode.Normal }
  });
  candleSeries = marketChart.addSeries(LightweightCharts.CandlestickSeries, {
    upColor: '#00a878', downColor: '#df5a62', borderVisible: false, wickUpColor: '#00a878', wickDownColor: '#df5a62'
  });
  volumeSeries = marketChart.addSeries(LightweightCharts.HistogramSeries, {
    priceFormat: { type: 'volume' }, priceScaleId: '', color: '#c7ded5'
  });
  marketChart.priceScale('').applyOptions({ scaleMargins: { top: 0.82, bottom: 0 } });
  chartResizeObserver = new ResizeObserver(() => marketChart?.resize(chartContainer.clientWidth, chartContainer.clientHeight));
...
async function loadHistory(symbol, period) {
  ...
  try {
    chartStatus.textContent = `Loading ${period}...`;
    const response = await fetch(apiUrl(`/api/stock-history?symbol=${encodeURIComponent(symbol)}&period=${period}`), { cache: 'no-store' });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error || `Chart API returned ${response.status}`);
    const values = Array.isArray(payload.values) ? payload.values : [];
    const candles = values.map(item => ({ time: Math.floor(new Date(item.datetime).getTime() / 1000), open: Number(item.open), high: Number(item.high), low: Number(item.low), close: Number(item.close) })).filter(item => Number.isFinite(item.time) && Number.isFinite(item.close)).sort((a, b) => a.time - b.time);
    ...
    if (!candles.length) throw new Error('No chart data returned');
    candleSeries.setData(candles);
    volumeSeries.setData(volumes);
    marketChart.timeScale().fitContent();
    const oldest = values[0];
    const latest = values[values.length - 1];
    document.getElementById('stat-open').textContent = formatMoney(latest.open);
    document.getElementById('stat-high').textContent = formatMoney(Math.max(...values.map(item => Number(item.high)).filter(Number.isFinite)));
    document.getElementById('stat-low').textContent = formatMoney(Math.min(...values.map(item => Number(item.low)).filter(Number.isFinite)));
    document.getElementById('stat-prev').textContent = formatMoney(oldest.close);
    chartStatus.textContent = `${values.length} candles • ${payload.source || 'market data'}`;
  } catch (error) {
...
function updatePrediction(stock) {
  ...
  document.getElementById('forecast-price').textContent = formatMoney(Number(stock.price) * (1 + change / 100 * 0.5));
  document.getElementById('forecast-change').textContent = `${change >= 0 ? '+' : ''}${(change * 0.5).toFixed(2)}% momentum signal`;
  document.getElementById('confidence-score').textContent = `${confidence}%`;
  document.getElementById('confidence-bar').style.width = `${confidence}%`;
}

const originalSetSelected = setSelectedStock;
setSelectedStock = function(stock) {
  originalSetSelected(stock);
...
document.querySelectorAll('#periods button').forEach(button => button.addEventListener('click', () => {
  ...
  if (selectedStock) loadHistory(selectedStock.symbol, selectedPeriod);
}));
...
marketSearchInput?.addEventListener('input', filterPriceCards);
priceRefresh?.addEventListener('click', loadPrices);
chartRefresh?.addEventListener('click', () => selectedStock && loadHistory(selectedStock.symbol, selectedPeriod));
...
async function loadNews() {
  ...
  try {
    const response = await fetch(apiUrl('/api/news?page=1&limit=6'), { cache: 'no-store' });
    if (!response.ok) throw new Error(`News API returned ${response.status}`);
    const payload = await response.json();
    const articles = Array.isArray(payload.data) ? payload.data : [];
    newsList.innerHTML = '';
    articles.forEach(article => { const card = typeof renderNewsArticle === 'function' ? renderNewsArticle(article) : null; if (card) newsList.appendChild(card); });
    if (newsStatus) newsStatus.textContent = articles.length ? `${articles.length} latest stories` : 'No news available';
  } catch (error) {
    console.error('News loading failed:', error);
    newsList.innerHTML = '<div class="news-empty">Unable to load news.</div>';
