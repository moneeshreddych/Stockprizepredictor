"""Latest completed daily close and multi-horizon ML prediction service."""
import os
...
def _fallback_market_status():
    ...
    if now.weekday() >= 5:
        return {"is_open": False, "holiday": None, "source": "schedule"}
    return {
    ...
    if FINNHUB_API_KEY:
        try:
            ...
            return {
                ...
                and payload.get("session") == "regular",
                "holiday": payload.get("holiday"),
                "source": "Finnhub",
            }
        except requests.RequestException:
            app.logger.exception(
                "Finnhub market-status request failed; using schedule fallback"
            )
...
def _completed_rows(history, symbols):
    now = datetime.now(MARKET_TZ)
    results = {}
    is_intraday = now.weekday() < 5 and now.time() < dt_time(16, 0)
    for symbol in symbols:
        try:
            ...
            if rows.empty:
                continue
            if is_intraday and rows.index[-1].date() == now.date():
                rows = rows.iloc[:-1]
            if rows.empty:
                continue
            close = float(rows.iloc[-1]["Close"])
            previous = float(rows.iloc[-2]["Close"]) if len(rows) > 1 else None
            ...
            app.logger.exception("Unable to parse closed daily data for %s", symbol)
    return results
    ...
    for start in range(0, len(SYMBOLS), BATCH_SIZE):
        ...
        try:
            ...
            if history is not None and not history.empty:
                results.update(_completed_rows(history, symbols))
        except Exception:
            app.logger.exception(
                "Unable to retrieve closed daily batch for %s", ",".join(symbols)
            )
    return results
...
def _load_supabase_cache():
    try:
        ...
        app.logger.exception("Unable to retrieve cached stock prices")
        return {}


def market_latest_prices():
    status = market_status()
    by_symbol = _fetch_latest_closed_batch()
    cached = _load_supabase_cache() if len(by_symbol) < len(SYMBOLS) else {}
    for symbol in SYMBOLS:
        if symbol not in by_symbol and symbol in cached:
            ...
            by_symbol[symbol] = {
                ...
                "change": float(old.get("change") or 0),
    ...
    return jsonify(
        {
            ...
            "price_type": "previous_close",
            "source": "Yahoo Finance",
            "received": sum(r.get("price") is not None for r in ordered),
            "expected": len(SYMBOLS),
            "missing_symbols": [
                r["symbol"] for r in ordered if r.get("price") is None
            ],
...
def predictions():
    ...
    try:
        query = (
            ...
            .select(
                "symbol,horizon,horizon_sessions,forecast_type,prediction_date,"
                "target_date,predicted_return,predicted_price,lower_return,"
                "upper_return,lower_price,upper_price,model_name,metrics,created_at"
            )
            .eq("model_name", "FinBERT+TFT-v2")
            .order("prediction_date", desc=True)
            .order("horizon_sessions")
            .order("created_at", desc=True)
        )
        if symbol:
            query = query.eq("symbol", symbol).limit(100)
        else:
        ...
        latest = {}
        for row in rows:
            key = (str(row["symbol"]).upper(), str(row.get("horizon") or ""))
