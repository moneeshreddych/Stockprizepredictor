...
def load_prices():
    ...
    if df.empty:
        raise RuntimeError(
            "No stock_prices rows found. Run news/price_collector.py first."
        )
    ...
    missing = [symbol for symbol in SYMBOLS if counts.get(symbol, 0) < ENCODER_LENGTH + 10]
    if missing:
        raise RuntimeError(
...
def score_finbert():
    ...
    cutoff = pd.Timestamp(
        datetime.now(ET).date() - timedelta(days=LOOKBACK_DAYS),
        tz="UTC",
    )
    ...
    with torch.inference_mode():
        for start in range(0, len(news), batch_size):
            ...
            for row, values in zip(batch.itertuples(index=False), probabilities):
                probability_map = {
                    labels[index]: float(values[index])
...
def load_sentiment():
    ...
    news = pd.DataFrame(
        fetch_all("news_articles", "id,published_at", 1000)
    )
    ...
    return (
        merged.groupby(["symbol", "date"], as_index=False)
        .agg(
...
def features(prices, sentiment):
    ...
    df["volatility_20"] = grouped["log_return"].transform(
    ...
    volume_mean = log_volume.groupby(df["symbol"]).transform(
        lambda x: x.rolling(20).mean()
    )
    volume_std = log_volume.groupby(df["symbol"]).transform(
        lambda x: x.rolling(20).std()
    )
    df["volume_z20"] = (log_volume - volume_mean) / volume_std.replace(
        0, np.nan
    )
...
def add_target(frame, horizon_sessions):
    ...
    df["target_close"] = df.groupby("symbol")["close"].shift(
        -horizon_sessions
    )
    return df.dropna(
        subset=UNKNOWN_FEATURES + ["target_return", "target_close"]
    ).copy()
...
def train_one(frame, horizon):
    ...
    # Keep the same date range across symbols so the global model does not
    # learn from a different calendar window for each stock.
    first_dates = supervised.groupby("symbol")["date"].min()
    supervised = supervised[
        supervised["date"] >= first_dates.max()
    ].copy()

    dates = sorted(supervised["date"].unique())
    if len(dates) < 120:
        raise RuntimeError(
            f"Not enough training dates for {horizon}: {len(dates)}"
        )
    ...
    validation_df = supervised[
        supervised["date"] >= validation_start
    ].copy()
    ...
    checkpoint = ModelCheckpoint(
        dirpath=str(ARTIFACTS),
        filename=f"tft-{horizon}-{{epoch:02d}}-{{val_loss:.5f}}",
        monitor="val_loss",
        mode="min",
        save_top_k=1,
    )
    early_stopping = EarlyStopping(
        monitor="val_loss",
        patience=int(os.getenv("TFT_EARLY_STOPPING_PATIENCE", "6")),
        mode="min",
    )
    ...
    model = TemporalFusionTransformer.from_dataset(
        ...
        hidden_size=int(os.getenv("TFT_HIDDEN_SIZE", "32")),
        attention_head_size=int(os.getenv("TFT_ATTENTION_HEADS", "4")),
        dropout=float(os.getenv("TFT_DROPOUT", "0.15")),
        hidden_continuous_size=int(
            os.getenv("TFT_HIDDEN_CONTINUOUS_SIZE", "16")
        ),
        lstm_layers=int(os.getenv("TFT_LSTM_LAYERS", "2")),
        output_size=3,
    ...
    trainer = Trainer(
        max_epochs=int(os.getenv("TFT_MAX_EPOCHS", "40")),
        accelerator="auto",
        devices=1,
        gradient_clip_val=float(
            os.getenv("TFT_GRADIENT_CLIP", "0.1")
        ),
    ...
    (ARTIFACTS / f"tft_{horizon}_metrics.json").write_text(
...
def predict_latest_tft(model, training, frame):
    """Return one probabilistic forecast for every symbol."""
    ...
    for symbol in SYMBOLS:
        symbol_frame = (
            ...
            .sort_values("date")
            .tail(ENCODER_LENGTH + 1)
            .copy()
        )
        if len(symbol_frame) < ENCODER_LENGTH + 1:
            continue

        # The latest row is the decoder row. Its target is intentionally
        # unknown; all features are information available at prediction time.
...
def publish_tft_predictions(frame, forecasts, horizon):
    ...
    for forecast in forecasts:
        ...
        target_date = (
            prediction_date + pd.offsets.BDay(horizon_sessions)
        ).date().isoformat()
        p50 = forecast["p50"]
        ...
        rows.append(
            {
                ...
                "metrics": {
                    ...
                    "sentiment": float(
                        frame.loc[
                            frame["symbol"] == forecast["symbol"],
                            "composite_score",
                        ].iloc[-1]
                    ),
                    "p10_return": forecast["p10"],
                    "p50_return": p50,
                    "p90_return": forecast["p90"],
                },
                "horizon": horizon,
                "horizon_sessions": horizon_sessions,
                "forecast_type": "tft_quantile",
                "lower_return": forecast["p10"],
                "upper_return": forecast["p90"],
    ...
    for start in range(0, len(rows), 50):
        ...
        if batch:
            supabase.table("predictions").upsert(
                batch,
                on_conflict=(
                    "stock_id,prediction_date,target_date,model_name,horizon"
                ),
            ).execute()
    ...
    for _, row in latest.iterrows():
        ...
        for horizon in SCENARIO_HORIZONS:
            ...
            target_date = (
                ...
                + pd.offsets.BDay(HORIZONS[horizon])
            ).date().isoformat()
            ...
            rows.append(
                {
                    ...
                    "target_date": target_date,
                    "predicted_return": center,
                    "predicted_price": price * np.exp(center),
                    "model_name": MODEL_NAME,
                    "metrics": {
                        "forecast_type": "scenario",
                        "data_quality": "scenario",
                        "long_term_cagr": cagr,
                    },
                    "horizon": horizon,
                    "horizon_sessions": HORIZONS[horizon],
                    ...
                    "lower_return": lower,
                    "upper_return": upper,
                    "lower_price": price * np.exp(lower),
                    "upper_price": price * np.exp(upper),
                }
            )
...
def train_tft(horizons=None):
    ...
    if not requested_tft:
        raise ValueError(
            "At least one TFT horizon is required: "
            + ", ".join(TFT_HORIZONS)
        )

    total = 0
    for horizon in requested_tft:
        model, training, metrics = train_one(frame, horizon)
        forecasts = predict_latest_tft(model, training, frame)
        total += publish_tft_predictions(frame, forecasts, horizon)

    # Long horizons are explicitly scenario forecasts rather than pretending
    # that a 1-day TFT model can reliably extrapolate for decades.
