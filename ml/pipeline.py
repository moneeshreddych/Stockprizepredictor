...
def train_one_horizon(frame, horizon, sessions):
    ...
    dates = sorted(target_frame["date"].unique()); train_end = dates[int(len(dates) * 0.70)]; val_end = dates[int(len(dates) * 0.85)]; train_frame = target_frame[target_frame["date"] <= train_end].copy()
    if len(train_frame) < 1000: raise RuntimeError(f"{horizon} has too few training rows: {len(train_frame)}")
    training = _make_tft_dataset(train_frame, "target_return"); val = _make_tft_dataset_from(training, target_frame[target_frame["date"] <= val_end].copy(), train_end); test = _make_tft_dataset_from(training, target_frame, val_end)
